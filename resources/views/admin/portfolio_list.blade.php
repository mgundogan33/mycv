                                ...
                                @foreach ($list as $item)
                                    <tr id="{{ $item->id }}">
                                        ...
                                        <td><a data-id="{{ $item->id }}" href="javascript:void(0)"
                                                class="btn btn-danger deleteEducation">Sil <i
                                                    class="fa fa-trash"></i></a>
                                        </td>

                                        <td>
                                            <a href="{{ route('portfolio.showImages', ['id' => $item->id]) }}">
                                                <img src="{{ $item->featuredImage ? asset('storage/portfolio/' . $item->featuredImage->image) : '-' }}"
                                                    width="150">
                                            </a>
                                        </td>
                                        ...
                                        <td>{{ $item->title }}</td>
                                        <td>{{ $item->tags }}</td>
                                        <td title="{{ strip_tags($item->about) }}">
                                            {{ strip_tags(substr($item->about, 0, 50)) }}</td>
                                        <td>
                                            @if ($item->status)
            ...
            $.ajax({
                ...
                success: function(response) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Başarılı !',
                        text: response.portfolioID + " ID'li kayıt durumu " + response
                            .newStatus + " olarak güncellenmiştir.",
